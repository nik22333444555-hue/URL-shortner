...
export function freshPlugin(i18next, options) {
  return function () {
...
export function missingKeyHandler(i18next) {
  ...
  return function (rq, rs) {
    ...
    if (!utils.isSafeLangIdentifier(lng) || !utils.isSafeNsIdentifier(ns)) {
      options.setStatus(res, 400);
      return options.send(res, 'i18next-http-middleware:: unsafe lng/ns value');
    }
    if (!i18next.services.backendConnector) {
      options.setStatus(res, 404);
      return options.send(res, 'i18next-express-middleware:: no backend configured');
